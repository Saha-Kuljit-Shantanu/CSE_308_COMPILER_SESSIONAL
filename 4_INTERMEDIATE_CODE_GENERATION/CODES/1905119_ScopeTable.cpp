...
public:
    ...
    bool INSERT(string name, string type,FILE* fp,bool isFunc,bool isArr, bool isDef,bool isGlobal,int len,int offset){
	...
	tmp -> getNext() -> setIsGlobal(isGlobal);
	tmp -> getNext() -> setLen(len);
	tmp -> getNext() -> setOffset(offset);

        fout << "        Inserted in ScopeTable# " << table_ID <<" at position " << h+1 << ", " <<ltmp << endl;
      cout << tmp -> getNext() -> getName() << tmp -> getNext() -> getOffset();
    ...
    bool INSERT(SymbolInfo* s,FILE* fp){
        ...
        long long int h = SDBM_Hash(s -> getName());
        ...
        long long int res = search_help(s -> getName(),scp[h] -> getNext());

        if( res != -1 ){

            fout << "        '" << s -> getName() << "' already exists in the current ScopeTable" << endl;
            fprintf(fp,"        %s already exists in the current ScopeTable\n",s -> getName().c_str());
            return false;
    ...
    void PRINT(FILE* fp){
        ...
        for(long long int i = 0; i< num_Buckets; i++){
            ...
            while(s -> getNext() != NULL){
                ...
                fprintf(fp,"<%s,%s%s%d> ", s -> getNext() -> getName().c_str(), str.c_str() , s -> getNext() -> getType().c_str(), s-> getNext() -> getOffset() );
                s = s -> getNext();
